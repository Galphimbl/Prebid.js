...
import {_each} from "../src/utils";
import {bidsBackCallback} from "../src/auction";
import {initAdServer} from "./adServer";
...
function initDFP(next, initParams) {
  logInfo('Init');
  if (!findDFPScript()) {
    loadDFPScript();
  }
  ...
  initParams.adUnits.filter((adUnit) => {
  ...
  initParams.bidsBackHandler = function (response) {
    window.googletag.cmd.push(function () {
      const dfpSlots = [];
      const gpt = window.googletag;
      _each(response, function(item, adUnitCode) {
        const {adUnitPath, sizes, phId} = dfpSlotsMap[adUnitCode];
        let dfpSlot = getDFPSlot(phId);
        if (!dfpSlot) {
          logInfo('define slot', `('${adUnitPath}', [...], '${phId}')`);
          dfpSlot = (oGptDefineSlot || gpt.defineSlot)(adUnitPath, sizes, phId);
          dfpSlot.addService(googletag.pubads());
          (oGptDisplay || gpt.display)(phId);
        }
        dfpSlots.push(dfpSlot);
        getGlobal().setTargetingForGPTAsync(phId);
        logInfo(`refresh "${phId}"`, dfpSlot);
      });
      gpt.pubads().refresh(dfpSlots);
    });
  }
  next(initParams);
}
initAdServer.before(initDFP);
